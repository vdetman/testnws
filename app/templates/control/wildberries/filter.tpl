		...
		<div class="row">
			...
			<div class="col-lg-2">
				...
				<select class="form-control autosubmit" name="Blocked">
					...
					<option value="sales" {if 'sales' == $filter->get('Blocked')}selected="selected"{/if}>Sales</option>
					<option value="prices" {if 'prices' == $filter->get('Blocked')}selected="selected"{/if}>Prices</option>
					<option value="incomes" {if 'incomes' == $filter->get('Blocked')}selected="selected"{/if}>Incomes</option>
				</select>
			</div>
		</div>
	</div>
